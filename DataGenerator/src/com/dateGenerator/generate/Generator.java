...
public class Generator {
	...
	public void generateJoinedRows(JoinedRow joinedRow, PatternRow row) {
		...
		for (PatternNode node : row.getPatternNodes()) {
			for (PatternRestriction restriction : getForeignRestrictions(node, row.getId())) {
				for(PatternNode restrictionNode: restriction.getPatternNodes()){
	...
	private List<PatternRestriction> getForeignRestrictions(PatternNode node ,int patternRowId) {
		List<PatternRestriction> result = new ArrayList<PatternRestriction>();
		for(PatternRestriction restriction : node.getPatternRestrictions()){
			for(PatternNode restrictionNode : restriction.getPatternNodes()){
